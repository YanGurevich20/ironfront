...
const ServerArenaActorUtilsData := preload("res://core/server_arena_actor_utils.gd")
...
func initialize_runtime(arena_level_packed_scene: PackedScene) -> bool:
	...
	print(
		(
			(
				"[server][arena-runtime] initialized level=res://levels/arena/arena_level_mvp.tscn "
				+ "count=%d bots=%d"
			)
