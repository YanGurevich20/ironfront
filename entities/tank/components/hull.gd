...
func setup_engine_sounds(engine_size_class: Enums.TankSizeClass) -> void:
	...
	engine_sound_slow.stream = engine_sounds_map[SPEED_TYPE.SLOW]
	engine_sound_normal.stream = engine_sounds_map[SPEED_TYPE.NORMAL]
	engine_sound_fast.stream = engine_sounds_map[SPEED_TYPE.FAST]
...
func setup_track_sounds(tank_size_class: Enums.TankSizeClass) -> void:
	...
	track_sound_slow.stream = track_sounds_map[SPEED_TYPE.SLOW]
	track_sound_normal.stream = track_sounds_map[SPEED_TYPE.NORMAL]
	track_sound_fast.stream = track_sounds_map[SPEED_TYPE.FAST]
...
func play_engine_sound(
	_left_track_input: float, _right_track_input: float, _linear_velocity: Vector2
) -> void:
	...
	var engine_sounds: Array[AudioStreamPlayer2D] = [
		engine_sound_slow,
		engine_sound_normal,
		engine_sound_fast,
	]
...
func play_track_sound(_linear_velocity: Vector2) -> void:
	...
	var track_sounds: Array[AudioStreamPlayer2D] = [
		track_sound_slow,
		track_sound_normal,
		track_sound_fast,
	]
...
func _get_sound_player_for_speed_type(
	speed_type: Enums.SpeedType, sound_type: SoundType
) -> AudioStreamPlayer2D:
...
func _crossfade_sounds(
	sounds: Array,
	active_sound: AudioStreamPlayer2D,
	pitch: float,
	fade_speed: float,
...
func stop_sounds() -> void:
	...
	for sound: AudioStreamPlayer2D in all_sounds:
		sound.stop()
