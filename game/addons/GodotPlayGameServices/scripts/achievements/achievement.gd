...
## Achievement type.
enum Type { TYPE_STANDARD = 0, TYPE_INCREMENTAL = 1 }  ## A standard achievement.  ## An incremental achievement.

## Achievement state.
enum State { STATE_UNLOCKED = 0, STATE_REVEALED = 1, STATE_HIDDEN = 2 }  ## An unlocked achievement.  ## A revealed achievement.  ## A hidden achievement.
...
func _init(dictionary: Dictionary) -> void:
	...
	if dictionary.has("player"):
		player = PlayGamesPlayer.new(dictionary.player)
