...
func _init(dictionary: Dictionary) -> void:
	...
	if dictionary.has("eventId"):
		event_id = dictionary.eventId
	if dictionary.has("formattedValue"):
		formatted_value = dictionary.formattedValue
	...
	if dictionary.has("isVisible"):
		is_visible = dictionary.isVisible
