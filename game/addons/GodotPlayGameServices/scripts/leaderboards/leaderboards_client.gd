...
signal player_centered_scores_loaded(
	leaderboard_id: String, leaderboard_scores: PlayGamesLeaderboardScores
)
...
func _connect_signals() -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.scoreSubmitted.connect(
			func(is_submitted: bool, leaderboard_id: String):
				score_submitted.emit(is_submitted, leaderboard_id)
		)
		GodotPlayGameServices.android_plugin.scoreLoaded.connect(
			func(leaderboard_id: String, json_data: String):
				var safe_dictionary := GodotPlayGameServices.json_marshaller.safe_parse_dictionary(
					json_data
				)
				score_loaded.emit(leaderboard_id, PlayGamesLeaderboardScore.new(safe_dictionary))
		)
		GodotPlayGameServices.android_plugin.allLeaderboardsLoaded.connect(
			func(leaderboards_json: String):
				var safe_array := GodotPlayGameServices.json_marshaller.safe_parse_array(
					leaderboards_json
				)
		...
		GodotPlayGameServices.android_plugin.leaderboardLoaded.connect(
			func(json_data: String):
				var safe_dictionary := GodotPlayGameServices.json_marshaller.safe_parse_dictionary(
					json_data
				)
				leaderboard_loaded.emit(PlayGamesLeaderboard.new(safe_dictionary))
		)
		GodotPlayGameServices.android_plugin.playerCenteredScoresLoaded.connect(
			func(leaderboard_id: String, json_data: String):
				var safe_dictionary := GodotPlayGameServices.json_marshaller.safe_parse_dictionary(
					json_data
				)
				player_centered_scores_loaded.emit(
					leaderboard_id, PlayGamesLeaderboardScores.new(safe_dictionary)
				)
		)
		GodotPlayGameServices.android_plugin.topScoresLoaded.connect(
			func(leaderboard_id: String, json_data: String):
				var safe_dictionary := GodotPlayGameServices.json_marshaller.safe_parse_dictionary(
					json_data
				)
				top_scores_loaded.emit(
					leaderboard_id, PlayGamesLeaderboardScores.new(safe_dictionary)
				)
		)
...
func show_leaderboard_for_time_span(
	leaderboard_id: String, time_span: PlayGamesLeaderboardVariant.TimeSpan
) -> void:
	if GodotPlayGameServices.android_plugin:
...
func show_leaderboard_for_time_span_and_collection(
	...
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.showLeaderboardForTimeSpanAndCollection(
			leaderboard_id, time_span, collection
		)
...
func load_player_centered_scores(
	...
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.loadPlayerCenteredScores(
			leaderboard_id, time_span, collection, max_results, force_reload
		)
