...
enum FriendsListVisibilityStatus {
	FEATURE_UNAVAILABLE = 3, REQUEST_REQUIRED = 2, UNKNOWN = 0, VISIBLE = 1  ## The friends list is currently unavailable for the game.  ## The friends list is not visible to the game, but the game can ask for access.  ## Unknown if the friends list is visible to the game, or whether the game can ask for access from the user.  ## The friends list is currently visible to the game.
}

## This player's friend status relative to the currently signed in player.
enum PlayGamesPlayerFriendStatus { FRIEND = 4, NO_RELATIONSHIP = 0, UNKNOWN = -1 }  ## The currently signed in player and this player are friends.  ## The currently signed in player is not a friend of this player.  ## The currently signed in player's friend status with this player is unknown.
...
func _init(dictionary: Dictionary) -> void:
	if dictionary.has("bannerImageLandscapeUri"):
		banner_image_landscape_uri = dictionary.bannerImageLandscapeUri
	if dictionary.has("bannerImagePortraitUri"):
		banner_image_portrait_uri = dictionary.bannerImagePortraitUri
	if dictionary.has("friendsListVisibilityStatus"):
		friends_list_visibility_status = FriendsListVisibilityStatus.get(
			dictionary.friendsListVisibilityStatus
		)
	if dictionary.has("displayName"):
		display_name = dictionary.displayName
	if dictionary.has("hiResImageUri"):
		hi_res_image_uri = dictionary.hiResImageUri
	if dictionary.has("iconImageUri"):
		icon_image_uri = dictionary.iconImageUri
	if dictionary.has("levelInfo"):
		level_info = PlayGamesPlayerLevelInfo.new(dictionary.levelInfo)
	if dictionary.has("playerId"):
		player_id = dictionary.playerId
	if dictionary.has("friendStatus"):
		friend_status = PlayGamesPlayerFriendStatus.get(dictionary.friendStatus)
	if dictionary.has("retrievedTimestamp"):
		retrieved_timestamp = dictionary.retrievedTimestamp
	if dictionary.has("title"):
		title = dictionary.title
	if dictionary.has("hasHiResImage"):
		has_hi_res_image = dictionary.hasHiResImage
	if dictionary.has("hasIconImage"):
...
func _to_string() -> String:
	...
	result.append(
		(
			"friends_list_visibility_status: %s"
			% FriendsListVisibilityStatus.find_key(friends_list_visibility_status)
		)
	)
