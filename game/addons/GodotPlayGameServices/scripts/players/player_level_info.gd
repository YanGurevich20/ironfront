...
func _init(dictionary: Dictionary) -> void:
	if dictionary.has("currentLevel"):
		current_level = PlayGamesPlayerLevel.new(dictionary.currentLevel)
	if dictionary.has("currentXpTotal"):
		current_xp_total = dictionary.currentXpTotal
	if dictionary.has("lastLevelUpTimestamp"):
		last_level_up_timestamp = dictionary.lastLevelUpTimestamp
	if dictionary.has("nextLevel"):
		next_level = PlayGamesPlayerLevel.new(dictionary.nextLevel)
	if dictionary.has("isMaxLevel"):
