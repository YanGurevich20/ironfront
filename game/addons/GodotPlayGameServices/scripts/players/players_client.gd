...
func _connect_signals() -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.friendsLoaded.connect(
			func(friends_json: String):
		...
		GodotPlayGameServices.android_plugin.playerSearched.connect(
			func(friend_json: String):
		...
		GodotPlayGameServices.android_plugin.currentPlayerLoaded.connect(
			func(friend_json: String):
				var safe_dictionary := GodotPlayGameServices.json_marshaller.safe_parse_dictionary(
					friend_json
				)
...
func load_friends(page_size: int, force_reload: bool, ask_for_permission: bool) -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.loadFriends(
			page_size, force_reload, ask_for_permission
		)
...
func compare_profile_with_alternative_name_hints(
	other_player_id: String, other_player_in_game_name: String, current_player_in_game_name: String
) -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.compareProfileWithAlternativeNameHints(
			other_player_id, other_player_in_game_name, current_player_in_game_name
		)
