...
func _connect_signals() -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.userAuthenticated.connect(
			func(is_authenticated: bool): user_authenticated.emit(is_authenticated)
		)
		GodotPlayGameServices.android_plugin.serverSideAccessRequested.connect(
			func(token: String): server_side_access_requested.emit(token)
		)
...
func request_server_side_access(server_client_id: String, force_refresh_token: bool) -> void:
	if GodotPlayGameServices.android_plugin:
		GodotPlayGameServices.android_plugin.requestServerSideAccess(
			server_client_id, force_refresh_token
		)
