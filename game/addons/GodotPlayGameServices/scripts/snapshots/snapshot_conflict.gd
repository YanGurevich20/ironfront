...
func _init(dictionary: Dictionary) -> void:
	...
	if dictionary.has("conflictId"):
		conflict_id = dictionary.conflictId
	if dictionary.has("conflictingSnapshot"):
		conflicting_snapshot = PlayGamesSnapshot.new(dictionary.conflictingSnapshot)
	if dictionary.has("serverSnapshot"):
		server_snapshot = PlayGamesSnapshot.new(dictionary.serverSnapshot)
