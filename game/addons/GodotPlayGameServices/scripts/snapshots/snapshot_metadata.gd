...
func _init(dictionary: Dictionary) -> void:
	if dictionary.has("snapshotId"):
		snapshot_id = dictionary.snapshotId
	if dictionary.has("uniqueName"):
		unique_name = dictionary.uniqueName
	if dictionary.has("description"):
		description = dictionary.description
	if dictionary.has("coverImageAspectRatio"):
		cover_image_aspect_ratio = dictionary.coverImageAspectRatio
	if dictionary.has("progressValue"):
		progress_value = dictionary.progressValue
	if dictionary.has("lastModifiedTimestamp"):
		last_modified_timestamp = dictionary.lastModifiedTimestamp
	if dictionary.has("playedTime"):
		played_time = dictionary.playedTime
	if dictionary.has("hasChangePending"):
		has_change_pending = dictionary.hasChangePending
	if dictionary.has("owner"):
		owner = PlayGamesPlayer.new(dictionary.owner)
	if dictionary.has("game"):
		game = PlayGamesGame.new(dictionary.game)
	if dictionary.has("deviceName"):
		device_name = dictionary.deviceName
	if dictionary.has("coverImageUri"):
		cover_image_uri = dictionary.coverImageUri
