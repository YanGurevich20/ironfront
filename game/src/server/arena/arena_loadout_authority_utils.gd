...
static func handle_peer_shell_select_request(
	...
	var shell_spec: ShellSpec = shell_select_request.get("shell_spec", null)
	var selected: bool = arena_session_state.apply_peer_shell_selection(peer_id, shell_spec)
	if not selected:
...
static func handle_peer_fire_request(
	...
	var selected_shell_spec: ShellSpec = arena_session_state.get_peer_selected_shell_spec(peer_id)
	var selected_shell_count: int = arena_session_state.get_peer_shell_count(
		peer_id, selected_shell_spec
	)
	...
	if not fired:
		...
		return
	var consumed: bool = arena_session_state.consume_peer_shell_ammo(peer_id, selected_shell_spec)
	if not consumed:
		push_warning(
			(
				"[server][arena-runtime] shell_consume_failed peer=%d shell=%s"
				% [peer_id, ShellManager.get_shell_id(selected_shell_spec)]
			)
		)
	sync_peer_tank_shell_state(runtime, peer_id, spawned_tank)
	send_peer_loadout_state(runtime, peer_id, spawned_tank)
...
static func sync_peer_tank_shell_state(
	...
	var selected_shell_spec: ShellSpec = runtime.arena_session_state.get_peer_selected_shell_spec(
		peer_id
	)
...
static func send_peer_loadout_state(
	...
	var selected_shell_id: String = (
		ShellManager.get_shell_id(selected_shell_spec) if selected_shell_spec != null else ""
	)
