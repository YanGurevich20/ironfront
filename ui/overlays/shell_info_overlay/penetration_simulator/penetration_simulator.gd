class_name PenetrationSimulator
extends Control
...
func _ready() -> void:
	Utils.connect_checked(
		shell_texture.gui_input,
		func(_event: InputEvent) -> void:
			if _event is InputEventScreenDrag:
				_update_shell_rotation()
	)
	Utils.connect_checked(armour_thickness_line_edit.text_submitted, _on_thickness_changed)
	Utils.connect_checked(simulate_button.pressed, _update_simulation)


func display_shell_info(shell_spec: ShellSpec) -> void:
	_shell_spec = shell_spec
...
func _update_shell_rotation() -> void:
	...
	var angle_to_impact_point: float = rad_to_deg(
		impact_point_marker.global_position.angle_to_point(global_mouse_position)
	)
...
func _update_simulation() -> void:
	var effective_thickness := _shell_spec.get_effective_thickness(
		impact_angle, armour_thickness_mm
	)
	var impact_result := _shell_spec.get_impact_result(impact_angle, armour_thickness_mm)
...
func _on_thickness_changed(new_text: String) -> void:
	var clamped_thickness: float = clamp(
		float(new_text), MIN_ARMOUR_THICKNESS_MM, MAX_ARMOUR_THICKNESS_MM
	)
	...
	armour_texture.size.y = clamp(
		MAX_ARMOUR_THICKNESS_PIXELS * thickness_ratio,
		MIN_ARMOUR_THICKNESS_PIXELS,
		MAX_ARMOUR_THICKNESS_PIXELS
	)
	_update_simulation()
...
func _update_labels(damage: float, result_name: String, effective_thickness: float) -> void:
	...
	var bounce_chance_string: String = (
		"%0.1f" % (_shell_spec.get_bounce_chance(impact_angle) * 100.0) + "%"
	)
	bounce_chance_label.text = (
		"BOUNCE CHANCE: "
		+ ("0% (Overmatched)" if result_name == "Overmatched" else bounce_chance_string)
	)
